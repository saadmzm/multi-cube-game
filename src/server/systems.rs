...
pub fn send_message_system(mut server: ResMut<RenetServer>, player_lobby: Res<PlayerLobby>) {
    ...
    let event = multi_cube_game::ServerMessage::LobbySync(lobby);
    ...
    for event in server_events.read() {
        match event {
            ServerEvent::ClientConnected { client_id } => {
                ...
                let message = bincode::serialize(&multi_cube_game::ServerMessage::PlayerJoin(*client_id)).unwrap();
            ...
            ServerEvent::ClientDisconnected { client_id, reason } => {
                ...
                let message = bincode::serialize(&multi_cube_game::ServerMessage::PlayerLeave(*client_id)).unwrap();
